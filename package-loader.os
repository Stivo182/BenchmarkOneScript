...
Процедура ОбработатьСтруктуруКаталоговПоСоглашению(Путь, СтандартнаяОбработка, Отказ)
	...
	ПутьКИсходникам = ОбъединитьПути(Путь, "src", "BenchmarkOneScript", "core");

	КаталогиКлассов = НайтиКаталогиПоИмени(ПутьКИсходникам, "Классы");
	КаталогиМодулей = НайтиКаталогиПоИмени(ПутьКИсходникам, "Модули");
	КаталогиПеречислений = НайтиКаталогиПоИмени(ПутьКИсходникам, "Перечисления");
	...
	Для Каждого мКаталог Из КаталогиПеречислений Цикл
...
Функция НайтиКаталогиПоИмени(Путь, ИмяКаталога)

	Результат = Новый Массив;

	ДобавитьКаталогиПоИмени(Путь, ИмяКаталога, Результат);

	Возврат Результат;

КонецФункции

Процедура ДобавитьКаталогиПоИмени(Путь, ИмяКаталога, Результат)
	...
	Для Каждого Файл Из Файлы Цикл
		...
		Если Файл.Имя = ИмяКаталога Тогда
			Результат.Добавить(Файл.ПолноеИмя);
		КонецЕсли;

		ДобавитьКаталогиПоИмени(Файл.ПолноеИмя, ИмяКаталога, Результат);
...
Процедура ДобавитьКлассЕслиРанееНеДобавляли(ПутьФайла, ИмяКласса)
    ...
    Исключение
        ...
        ИскомаяОшибкаRu = СтрШаблон("Тип не зарегистрирован (%1)", ИмяКласса);
        ИскомаяОшибкаEn = СтрШаблон("Type is not registered (%1)", ИмяКласса);
        КлассУжеЕсть = СтрНайти(СообщениеОшибки, ИскомаяОшибкаRu) = 0 И СтрНайти(СообщениеОшибки, ИскомаяОшибкаEn) = 0;
    КонецПопытки;
