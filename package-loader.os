...
Процедура ОбработатьСтруктуруКаталоговПоСоглашению(Путь, СтандартнаяОбработка, Отказ)
	...
	ПутьКИсходникам = ОбъединитьПути(Путь, "src", "BenchmarkOneScript");

	КаталогиКлассов = Новый Массив;
	КаталогиКлассов.Добавить(ОбъединитьПути(ПутьКИсходникам, "core", "Классы"));

	КаталогиМодулей = Новый Массив;
	КаталогиМодулей.Добавить(ОбъединитьПути(ПутьКИсходникам, "core", "Модули"));
