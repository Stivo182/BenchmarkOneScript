...
&Опция(Имя = "p param", Описание = "Общий параметр бенчмарков в формате <Поле>=<Значение>")
&ТМассивСтрок
Перем _ПараметрыCLI; // Массив из Строка
...
Функция НайтиФайлыБенчмарков()
	...
	Для Каждого ИмяФайла Из _МассивФайлов Цикл
		...
		Иначе
			...
			ПодготовленныйФайл = _ПодготовительФайлаБенчмарка.Подготовить(ПолноеИмяФайла);
			Файлы.Добавить(ПодготовленныйФайл);
		КонецЕсли;
	КонецЦикла;
...
Функция ПрочитатьКонфигурацию(Настройки)
	...
	_ПарсерПараметровБенчмарковCLI.Применить(_ПараметрыCLI, Конфигурация);
