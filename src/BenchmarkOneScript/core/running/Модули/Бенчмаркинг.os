...
Функция Запустить(ИсточникБенчмарков, Конфигурация = Неопределено) Экспорт
	...
	Репортер = Новый КонсольныйРепортерБенчмарков();
	
	ЗапускательБенчмарков = Новый ЗапускательБенчмарков(ИсточникБенчмарков, Конфигурация, Репортер);
	РезультатЗапуска = ЗапускательБенчмарков.Запустить();

	ПубликаторРезультатаЗапуска = Новый ПубликаторРезультатаЗапускаБенчмарков(Репортер);
...
// Возвращаемое значение:
//   ТаблицаЗначений:
//     * Результат      - РезультатЗапускаБенчмарков, Неопределено - см. Бенчмаркинг.Запустить
...
Функция ЭтоДочернийПроцесс() Экспорт
	Возврат ПолучитьПеременнуюСреды("BENCHOS_IS_CHILD_PROCESS") = "1";
КонецФункции
...
Функция ИмяТипаИзСтроки(Строка)
	...
	Если _РегуляркаЗапрещенныеСимволыВИмени = Неопределено Тогда
		_РегуляркаЗапрещенныеСимволыВИмени = Новый РегулярноеВыражение("^[0-9]+|[^a-zA-Zа-яА-Я0-9_]");
	КонецЕсли;
