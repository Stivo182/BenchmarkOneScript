...
// Экспортирует результат запуска бенчмарков в файл HTML
...
Процедура Записать(РезультатЗапускаБенчмарков, ПолноеИмяФайла) Экспорт
	...
	Части.Добавить("<body>");
	Части.Добавить(СтрШаблон("<pre><code>%1</code></pre>", Строка(РезультатЗапускаБенчмарков.СредаОкружения)));

	Части.Добавить("<table>");
	Части.Добавить("<thead>");
	Части.Добавить("<tr>");
	ВидимыеКолонки = РезультатЗапускаБенчмарков.Отчет.ВидимыеКолонки();
	...
	Для Каждого СтрокаОтчета Из РезультатЗапускаБенчмарков.Отчет.Таблица() Цикл

		Части.Добавить("<tr>");

		Для Каждого Колонка Из ВидимыеКолонки Цикл
			ПараметрыПредставления = РезультатЗапускаБенчмарков.Отчет.ПараметрыПредставленияКолонки(Колонка);
			РезультатЗапускаКейса = РезультатЗапускаБенчмарков.Отчет.РезультатЗапускаКейсаСтроки(СтрокаОтчета);
			ПредставлениеЗначения = Колонка.ПредставлениеЗначения(
				СтрокаОтчета[Колонка.Имя],
				РезультатЗапускаКейса,
				ПараметрыПредставления
			);

			Части.Добавить("<td>");
			Части.Добавить(ПредставлениеЗначения);
			Части.Добавить("</td>");
		КонецЦикла;
