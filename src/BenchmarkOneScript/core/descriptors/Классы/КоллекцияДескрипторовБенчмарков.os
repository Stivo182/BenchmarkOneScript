...
//
// Параметры:
//   Тип - Тип - Тип класса бенчмарков, откуда будут прочитаны дескрипторы
...
Функция НайтиПоТипуИИмени(Тип, Имя) Экспорт
	...
	Возврат ПроцессорыКоллекций
		.ИзКоллекции(_ДескрипторыБенчмарков)
		.Фильтровать("Элемент -> Элемент.Метод() = Имя И Элемент.ТипКласса() = Тип", Контекст)
		.ПолучитьПервый();
...
Процедура Прочитать(Тип) Экспорт
	
	Очистить();

	Извлекатель = Новый ИзвлекательДескрипторовБенчмарков(Тип);
