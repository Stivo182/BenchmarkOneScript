...
Процедура ПриСозданииОбъекта(Тип)
	_Тип = Тип;
КонецПроцедуры
...
Функция Извлечь(Конфигурация = Неопределено) Экспорт
	...
	РефлекторОбъекта = Новый РефлекторОбъекта(_Тип);
...
Процедура ПрочитатьВсеОбработчикиСобытий(РефлекторОбъекта, Конфигурация)
	...
	ПрочитатьОбработчикиСобытия(СобытияБенчмарков.ПередКаждымКейсом, РефлекторОбъекта, Конфигурация);
	ПрочитатьОбработчикиСобытия(СобытияБенчмарков.ПослеКаждогоКейса, РефлекторОбъекта, Конфигурация);
	ПрочитатьОбработчикиСобытия(СобытияБенчмарков.ПередИтерацией, РефлекторОбъекта, Конфигурация);
	ПрочитатьОбработчикиСобытия(СобытияБенчмарков.ПослеИтерации, РефлекторОбъекта, Конфигурация);

	// Обратная совместимость
	ПрочитатьОбработчикиСобытия("ПередКаждым", РефлекторОбъекта, Конфигурация, СобытияБенчмарков.ПередКаждымКейсом);
	ПрочитатьОбработчикиСобытия("ПослеКаждого", РефлекторОбъекта, Конфигурация, СобытияБенчмарков.ПослеКаждогоКейса);
...
//
// Параметры:
//   ИмяАннотации     - Строка                 - Имя аннотации для поиска методов
//   РефлекторОбъекта - РефлекторОбъекта       - Рефлектор источника бенчмарков
//   Конфигурация     - КонфигурацияБенчмарков - Конфигурация для заполнения
//   ИмяСобытия       - Строка                 - Имя события для регистрации.
//                                               Если не указано, используется ИмяАннотации
Процедура ПрочитатьОбработчикиСобытия(ИмяАннотации, РефлекторОбъекта, Конфигурация, ИмяСобытия = "")

	ТаблицаМетодов = РефлекторОбъекта.ПолучитьТаблицуМетодов(ИмяАннотации);
	Если ТаблицаМетодов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(ИмяСобытия) Тогда
		ИмяСобытия = ИмяАннотации;
	КонецЕсли;
...
Процедура ПрочитатьАннотацииКолонок(СвойстваКонструктора, Конфигурация)
	...
	ПрочитатьАннотацииПользовательскихКолонок(СвойстваКонструктора, Конфигурация);
...
// Читает аннотации &Колонка и добавляет пользовательские колонки в конфигурацию
//
// Параметры:
//   СвойстваКонструктора - СтрокаТаблицыЗначений  - Свойства метода ПриСозданииОбъекта
//   Конфигурация         - КонфигурацияБенчмарков - Конфигурация для заполнения
Процедура ПрочитатьАннотацииПользовательскихКолонок(СвойстваКонструктора, Конфигурация)

	Аннотации = РаботаСАннотациями.ПолучитьАннотации(СвойстваКонструктора, "Колонка");

	Для Каждого Аннотация Из Аннотации Цикл
		ИмяКолонки = РаботаСАннотациями.ПолучитьЗначениеПараметраАннотации(Аннотация);
