...
Функция Запустить() Экспорт
	...
	Если ЗапускатьВТекущейСреде Тогда
		Результат = ЗапуститьВТекущейСреде();
	Иначе
		Результат = ЗапуститьВДругихСредах();
	КонецЕсли;
...
Функция ЗапуститьВДругихСредах()
	...
	Результат.Отчет = Новый ПостроительОтчетаБенчмарков(
...
Функция ПодготовитьПроцесс(Конфигурация, Версия)
	...
	Ожидаем.Что(Дескриптор, "Не найдены бенчмарки").Заполнено();

	Тип = Дескриптор.ТипОбъекта();
	СтрокаКоманды = Новый Массив();
	
	// oscript.exe
	Если ФС.ФайлСуществует(Версия) Тогда
		СтрокаКоманды.Добавить(Версия);
	Иначе
		СтрокаКоманды.Добавить(ИмяФайлаИсполняющейСреды(Версия));
	КонецЕсли;

	// benchos
	ПутьКСценарию = Новый Файл(ОбъединитьПути(ТекущийСценарий().Каталог, "../../cmd/main.os")).ПолноеИмя;
	СтрокаКоманды.Добавить(ОбернутьВКавычки(ПутьКСценарию));
	...
	СтрокаКоманды.Добавить(ОбернутьВКавычки(ФайлКонфигурацииДляПроцесса(Конфигурация)));

	// FILE
	СтрокаКоманды.Добавить(ОбернутьВКавычки(Новый РасширениеТипа(Тип).Источник));
...
Функция ФайлКонфигурацииДляПроцесса(Конфигурация)
...
Функция ИмяФайлаИсполняющейСреды(Версия)
	...
	Если Не ФС.КаталогСуществует(КаталогOVM) Тогда
		ВызватьИсключение "Не найден каталог с установленными версиями OneScript. Необходимо установить OneScript через OVM";
	КонецЕсли;
...
Функция ЗапуститьВТекущейСреде()
...
Функция ЗапуститьВсеБенчмарки()
	...
	Результат.Отчет = ПостроительОтчета.Сформировать();
...
Процедура ВывестиЗаголовок(Версия = "")
	...
	ВывестиСтроку(" ");
	ВывестиСтроку(СтрШаблон("*** Запуск бенчмарков в среде OneScript <%1> ***", Версия), "Синий");
