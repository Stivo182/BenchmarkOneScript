...
Функция Запустить(Кейсы = Неопределено) Экспорт
	
	// TODO: Проверка кейсов в валидаторе
	...
	РезультатЗапуска = ПодготовитьИЗапуститьБенчмарки(Кейсы);
...
Функция ПодготовитьИЗапуститьБенчмарки(Кейсы)
...
Процедура ЗапуститьКейсыБенчмарков(Кейсы, РезультатЗапуска)
	...
	Если Кейсы = Неопределено Тогда
		...
		Кейсы = ГенераторКейсов.Сгенерировать();
	КонецЕсли;
