...
Перем _ТекущийИндексПараметров; // Число
...
//   Чекпоинт - ЧекпоинтЗапускаБенчмарков - Чекпоинт запуска
Процедура ПриСозданииОбъекта(ИсточникБенчмарков, Конфигурация = Неопределено, Чекпоинт = Неопределено)

	РезолверБенчмарков = Новый РезолверБенчмарков(ИсточникБенчмарков);
	
	_ОбъектБенчмарков        = РезолверБенчмарков.ОбъектБенчмарков();
	...
	Иначе
		_Конфигурация = Новый КонфигурацияБенчмарков(РезолверБенчмарков.ТипКласса());
	КонецЕсли;
...
Функция ПодготовитьИЗапуститьБенчмарки()
	...
	// Запуск бенчмарков
	ЗапуститьБенчмарки(РезультатЗапуска);
...
Процедура ЗапуститьБенчмарки(РезультатЗапуска)
	...
	Для Каждого ДескрипторБенчмарка Из _ДескрипторыБенчмарков.ВМассив() Цикл
		...
		Если ТребуетсяПропуститьБенчмарк(ДескрипторБенчмарка) Тогда
			_Лог.Отладка("Пропущен бенчмарк <%1>", ДескрипторБенчмарка.Метод());
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Параметры Из КомбинацииПараметров Цикл
			...
			_ТекущийИндексПараметров = _ТекущийИндексПараметров + 1;

			ПроверитьИСоздатьТочкуВосстановления(ДескрипторБенчмарка.Метод(), _ТекущийИндексПараметров);
			
			Если _НовыйЧекпоинт <> Неопределено Тогда
				Возврат;
			КонецЕсли;

			Если ТребуетсяПропуститьПараметры(_ТекущийИндексПараметров) Тогда
				_Лог.Отладка("Пропущены параметры индекс <%1>", _ТекущийИндексПараметров);
				Продолжить;
			КонецЕсли;
...
Функция ЗапуститьБенчмарк(ДескрипторБенчмарка, Параметры)
	...
	Если ЭтоПервыйЗапускКейса() Тогда
	...
	Если Не ИзолироватьКаждыйЗапускКейса() Тогда
...
Процедура ЗапуститьЭтапИзмерения(Делегат, КоличествоВызововЗаИтерацию, Замеры)
	...
	Для НомерИтерации = 1 По _Конфигурация.КоличествоИтераций() Цикл
		...
		Если РезультатИтерации <> Неопределено Тогда
...
Функция ВыполнитьИтерацию(Делегат, Этап, НомерИтерации, КоличествоВызовов)
	...
	ПроверитьИСоздатьТочкуВосстановления(Делегат.ИмяМетода(), _ТекущийИндексПараметров, Этап, НомерИтерации);
	
	Если ТребуетсяПропуститьИтерацию(Этап, НомерИтерации) Тогда
...
Функция ЭтоПервыйЗапускКейса()
	Возврат _Чекпоинт = Неопределено 
		Или _Чекпоинт.Этап = ЭтапыБенчмарка.Измерение И _Чекпоинт.НомерИтерации = 1;
КонецФункции

Процедура ПроверитьИСоздатьТочкуВосстановления(ИмяМетода, ИндексПараметров, Этап = "", НомерИтерации = 1)
	...
	_НовыйЧекпоинт.ИмяМетода = ИмяМетода;
	_НовыйЧекпоинт.ИндексПараметров = ИндексПараметров;
...
Функция ТребуетсяПропуститьБенчмарк(ДескрипторБенчмарка)	

	Возврат _НовыйЧекпоинт = Неопределено 
		И _Чекпоинт <> Неопределено
		И _Чекпоинт.ИмяМетода <> ДескрипторБенчмарка.Метод()
		И Не ТребуетсяЗапускНовогоПроцесса();

КонецФункции

Функция ТребуетсяПропуститьПараметры(ИндексПараметров)	

	Возврат _Чекпоинт <> Неопределено
		И _Чекпоинт.ИндексПараметров > ИндексПараметров;
