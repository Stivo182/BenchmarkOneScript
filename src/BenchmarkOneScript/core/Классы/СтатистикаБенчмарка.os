...
//
// Параметры:
//   Замеры - ЗапускательБенчмарков.НоваяТаблицаЗамеров
Процедура ПриСозданииОбъекта(Замеры = Неопределено)

	_ИсходныеДанные = Новый Структура();
	_ИсходныеДанные.Вставить("Среднее", 0);
	_ИсходныеДанные.Вставить("СтандартноеОтклонение", 0);
	_ИсходныеДанные.Вставить("СтандартнаяОшибкаСреднего", 0);
	_ИсходныеДанные.Вставить("Медиана", 0);
...
Функция Прочитать(Замеры) Экспорт
	...
	ЗамерыВремени = ВыгрузитьЗамеры(Замеры, "НаносекундЗаОперацию", ЭтапыБенчмарка.Результат);
	ВыделяемаяПамять = ВыгрузитьЗамеры(Замеры, "ВыделяемаяПамятьЗаОперацию", ЭтапыБенчмарка.Память);
	...
	ВыделяемаяПамять = МатематическиеФункцииБенчмарков.Среднее(ВыделяемаяПамять);
