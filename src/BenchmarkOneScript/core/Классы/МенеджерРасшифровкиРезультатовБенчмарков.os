...
Процедура ОпределитьЭталоны(Конфигурация) Экспорт
	...
	СтрокаЭталон = Конфигурация.ВерсииИсполняющейСреды().Найти(Истина, "ЭтоЭталон");
	ЭталоннаяВерсия = ?(СтрокаЭталон = Неопределено, "", СтрокаЭталон.Версия);
...
Функция НайтиЭталонДляСтроки(ВходнаяСтрока, ЭталоннаяВерсияИсполняющейСреды)
	...
	Для Каждого СтрокаЭталон Из _Таблица Цикл
		...
		НайденЭталон = ЭтоРазныеСтроки 
			...
			И (
				ЭталоннаяВерсияИсполняющейСредыУказана И ЭталоннаяВерсияИсполняющейСредыСовпадает И ИмяМетодаСовпадает
				Или
				Не ЭталоннаяВерсияИсполняющейСредыУказана И ЭтоЭталон И КатегорияСовпадает И ВерсияИсполняющейСредыСовпадает
			);
