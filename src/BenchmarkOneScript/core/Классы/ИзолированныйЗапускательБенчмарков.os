...
Функция ЗапуститьВИсполняющейСреде(Конфигурация, ИсполняющаяСреда)
	...
	РезультатЗапуска = ПрочитатьРезультатЗапускаБенчмарковИзJSON(КаталогАртефактов, ИсполняющаяСреда);
...
Функция ПодготовитьПроцесс(Конфигурация, Версия)
	...
	СтрокаКоманды.Добавить(ПутьИсполняемогоФайла);
...
Процедура ПодготовитьКонфигурационныйФайл(КаталогСохранения)
	...
	КаталогиБиблиотек = ПолучитьЗначениеСистемнойНастройки("lib.system");
	ДополнительныеКаталогиБиблиотек = ПолучитьЗначениеСистемнойНастройки("lib.additional");

	ДополнитьНастройкуКонфигурации(ПрочитанныеНастройки, "lib.additional", ДополнительныеКаталогиБиблиотек);
	ДополнитьНастройкуКонфигурации(ПрочитанныеНастройки, "lib.additional", КаталогиБиблиотек);
...
Процедура ДополнитьНастройкуКонфигурации(Настройки, Ключ, Значение, Разделитель = ";")

	Если Не ЗначениеЗаполнено(Значение) Тогда
		Возврат;
	КонецЕсли;

	ИсходноеЗначеие = Настройки[Ключ];
	Если ЗначениеЗаполнено(ИсходноеЗначеие) Тогда
		НовоеЗначение = ИсходноеЗначеие + Разделитель + Значение;
	Иначе
		НовоеЗначение = Значение;
	КонецЕсли;

	Настройки[Ключ] = НовоеЗначение;
...
Процедура ПрочитатьКонфигурационныйФайл(ПутьКФайлу, ПрочитанныеНастройки)
	...
	Для Каждого Совпадение Из Совпадения Цикл
		...
		Если Ключ = "lib.system" Или Ключ = "lib.additional" Тогда
			...
			Значение = СтрСоединить(РазрешитьПутиФС(Пути, КаталогРазрешенияПутей), ";");
		КонецЕсли;
...
Функция ОпределитьИсполняемыйФайл(Версия)
	...
	Если ПутьФайла = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Не найдена установленная версия OneScript (%1). Установите её командой: ovm install %1", Версия);
	КонецЕсли;
...
Функция ПрочитатьРезультатЗапускаБенчмарковИзJSON(Каталог, ИсполняющаяСреда)
...
Функция НайтиПоследнююОшибку()
	...
	Для ИндексСообщения = 1 По КоличествоПроверяемыхСообщений Цикл
