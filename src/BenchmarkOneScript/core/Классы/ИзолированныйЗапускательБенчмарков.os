...
Функция ПодготовитьПроцесс(Конфигурация, Версия)
	...
	СтрокаКоманды.Добавить(ОбернутьВКавычки(ПутьИсполняемогоФайла));
...
Процедура ПодготовитьКонфигурационныйФайл(КаталогСохранения)
	...
	ИмяПараметраБиблиотек = "lib.additional";
	...
	ДополнительныеКаталогиБиблиотек = ПолучитьЗначениеСистемнойНастройки(ИмяПараметраБиблиотек);

	ДополнитьНастройкуКонфигурации(ПрочитанныеНастройки, ИмяПараметраБиблиотек, ДополнительныеКаталогиБиблиотек);
	ДополнитьНастройкуКонфигурации(ПрочитанныеНастройки, ИмяПараметраБиблиотек, КаталогиБиблиотек);
...
Процедура ДополнитьНастройкуКонфигурации(Настройки, Ключ, Значение, Разделитель = ";")
	...
	ИсходноеЗначение = Настройки[Ключ];
	Если ЗначениеЗаполнено(ИсходноеЗначение) Тогда
		НовоеЗначение = ИсходноеЗначение + Разделитель + Значение;
	Иначе
