...
//
// Параметры:
//   Обработчик - ОбработчикСобытияБенчмарка
//
// Возвращаемое значение:
//   ЭтотОбъект
Функция ДобавитьОбработчикСобытия(Обработчик) Экспорт

	Если _Обработчики.Найти(Обработчик) <> Неопределено Тогда
		Возврат ЭтотОбъект;
	КонецЕсли;

	_Обработчики.Добавить(Обработчик);
...
Процедура ПрименитьМодифицированныеОбработчикиСобытий(ЗначенияМодифицированныхПолей)
	...
	Для Каждого Обработчик Из ЗначенияМодифицированныхПолей[ИмяПоля_ОбработчикиСобытий()] Цикл
		ДобавитьОбработчикСобытия(Обработчик);
	КонецЦикла;
