...
&Сериализуемое("Экспортеры")
&Тип("Массив")
Перем _ЭкспортерыСериализуемое; // Массив из Строка

&Сериализуемое("ВерсииИсполняющейСреды")
&Тип("ТаблицаЗначений")
...
Функция ВТаблицу() Экспорт
	...
	Для Каждого ОписаниеПоля Из ОписанияПолей() Цикл
		...
		СтрокаТаблицы.Значение = _Рефлектор.ПолучитьСвойство(ЭтотОбъект, ОписаниеПоля.ИмяВнутреннее);
...
Процедура ОбработатьПередСериализацией() Экспорт

	ПроверитьОбработчикиСобытийПередСериализацией();
	ПроверитьИсточникиПараметровПередСериализацией();
	ОбработатьЭкспортерыПередСериализацией();

КонецПроцедуры

Процедура ОбработатьПослеДесериализации() Экспорт
	
	ОбработатьПараметрыПослеДесериализации();
	ОбработатьЭкспортерыПослеДесериализации();
	ОпределитьМодифицированныеПоля();
	ВыполнитьВалидациюПослеДесериализации();
...
Процедура ПроверитьОбработчикиСобытийПередСериализацией() Экспорт
	
	Для Каждого Строка Из _Обработчики Цикл
...
Процедура ОбработатьЭкспортерыПередСериализацией()

	_ЭкспортерыСериализуемое = Новый Массив;

	Для Каждого Экспортер Из _Экспортеры Цикл
		_ЭкспортерыСериализуемое.Добавить(Строка(ТипЗнч(Экспортер)));
	КонецЦикла;
	
КонецПроцедуры

Процедура ОбработатьЭкспортерыПослеДесериализации()
	
	Для Каждого СтрокаТип Из _ЭкспортерыСериализуемое Цикл
		ДобавитьЭкспортер(Новый(СтрокаТип));
	КонецЦикла;

КонецПроцедуры

Процедура ОбработатьПараметрыПослеДесериализации()
	
	Параметры = Новый ФиксированныйМассив(_Параметры);

	ОчиститьПараметры();
	Для Каждого ПараметрДанные Из Параметры Цикл
		ДобавитьПараметр(ПараметрДанные.Имя, ПараметрДанные.Значение);
	КонецЦикла;

КонецПроцедуры

Процедура ВыполнитьВалидациюПослеДесериализации()
