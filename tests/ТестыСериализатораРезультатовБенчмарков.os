...
&Тест
Процедура ТестДолжен_СериализоватьИДесериализоватьРезультатЗапускаБенчмарков() Экспорт
	...
	Конфигурация
		...
		.ДобавитьИсполняющуюСреду("dev", "develop");

	// Действие
	Результат = Бенчмаркинг.Запустить(ДескрипторыБенчмарков, Конфигурация);

	// Проверка
	ИмяФайла = _МенеджерВременныхФайлов.НовоеИмяФайла();
	...
	Ожидаем.Что(ДескрипторБенчмарка.ТипКласса(), "Тип").Равно(Тип);
	...
	Ожидаем.Что(Запуски[0].Кейс.ДескрипторБенчмарка(), "Должен быть создан один объект дескриптора").Равно(Запуски[1].Кейс.ДескрипторБенчмарка());
	Ожидаем.Что(Запуски[0].Кейс.ДескрипторБенчмарка(), "Дескриптор из запусков должен совпадать с дескриптором из коллекции дескрипторов").Равно(ДескрипторБенчмарка);
	Ожидаем.Что(Запуски[0].Кейс.Параметры(), "Количество строк в параметрах").ИмеетДлину(3);
	Ожидаем.Что(Запуски[0].Кейс.Параметры()[0].Значение(), "Параметр 1.1").Равно('19700101'); // Дата
	Ожидаем.Что(Запуски[0].Кейс.Параметры()[1].Значение(), "Параметр 1.2").Равно(3);
	Ожидаем.Что(Запуски[0].Кейс.Параметры()[2].Значение(), "Параметр 1.3").Равно(4);
	Ожидаем.Что(Запуски[1].Кейс.Параметры()[0].Значение(), "Параметр 2.1").Равно(Неопределено); // Неопределено
	Ожидаем.Что(Запуски[2].Кейс.Параметры()[0].Значение(), "Параметр 3.1").ИмеетТип("Массив"); // ТаблицаЗначений
	Ожидаем.Что(Запуски[2].Кейс.Параметры()[0].Значение()[0].Колонка, "Параметр 3.1").Равно(1);
	Ожидаем.Что(Запуски[3].Кейс.Параметры()[0].Значение(), "Параметр 4.1").Равно(Неопределено); // ЧтениеXML
	Ожидаем.Что(Запуски[3].Кейс.Параметры()[0].Представление(), "Параметр 4.1").Равно("ЧтениеXML"); // ЧтениеXML
	Ожидаем.Что(Запуски[4].Кейс.Параметры()[0].Значение(), "Параметр 5.1").ИмеетТип("Структура"); // БенчмаркСПараметрамиМетодаИПоля
	Ожидаем.Что(Запуски[4].Кейс.Параметры()[0].Представление(), "Параметр 5.1").Равно("БенчмаркСПараметрамиМетодаИПоля"); // БенчмаркСПараметрамиМетодаИПоля
	...
	Ожидаем.Что(ИсполняющиеСреды, "Исполняющие среды").ИмеетДлину(1);
	Ожидаем.Что(ИсполняющиеСреды[0].Версия, "Исполняющая среда 1").Заполнено();
	Ожидаем.Что(ИсполняющиеСреды[0].Наименование, "Исполняющая среда 1").Равно("develop");
	Ожидаем.Что(ИсполняющиеСреды[0].ЭтоЭталон, "Исполняющая среда 1").ЭтоЛожь();
