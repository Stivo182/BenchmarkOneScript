...
&Тест
Процедура Тест_ДолженПроверитьСериализациюИДесериализацию() Экспорт
...
Процедура ПроверитьДескрипторыБенчмаркаСАннотациямиКонфигурации(ДескрипторыБенчмарков)
	...
	Дексриптор = ДескрипторыБенчмарков.ПолучитьПервый();
	НаборыПараметров = Дексриптор.НаборыПараметров();
	ИсточникиПараметров = Дексриптор.ИсточникиПараметров();

	Ожидаем.Что(Дексриптор.ТипОбъекта()).Равно(Тип("БенчмаркСАннотациямиКонфигурации"));
	Ожидаем.Что(Дексриптор.Метод()).Равно("Бенчмарк");
	Ожидаем.Что(Дексриптор.ЭтоЭталон()).ЭтоЛожь();
