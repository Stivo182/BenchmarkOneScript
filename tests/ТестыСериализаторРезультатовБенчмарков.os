...
Процедура ТестДолжен_СериализоватьИДесериализовать() Экспорт
	...
	Конфигурация
		...
		.ДобавитьВерсиюИсполняющейСреды("2.0.0", "dev");
	...
	Ожидаем.Что(Запуски[0].ДескрипторБенчмарка, "Дескриптор из запусков должен совпадать с дескриптором из коллекции декскрипторов").Равно(ДескрипторБенчмарка);
	...
	Ожидаем.Что(Запуски[0].ИсполняющаяСреда.Версия, "Версия").Равно("2.0.0");
	...
	Ожидаем.Что(Конфигурация.ВерсииИсполняющейСреды()[0].Версия, "Исполняющая среда 1").Равно("2.0.0");
	Ожидаем.Что(Конфигурация.ВерсииИсполняющейСреды()[0].Наименование, "Исполняющая среда 1").Равно("dev");
