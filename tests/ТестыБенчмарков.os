...
Процедура ТестДолжен_СортироватьОтчетОтБыстрыхКМедленнымИзАннотации() Экспорт
	...
	Обработчик = Новый ОбработчикСобытияБенчмарка(
		"ОбработчикиСобытийБенчмарков.УстановкаВремени",
		СобытияБенчмарков.ПослеКаждого);

	Конфигурация.ДобавитьОбработчикСобытия(Обработчик);
