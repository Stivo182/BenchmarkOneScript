...
Процедура Тест_ЗапускБенчмарковИзНесуществующегоКаталога() Экспорт
	...
	Каталог = "./tests/fixtures/Классы/НесуществуюущийКаталог";
	Результат = Бенчмаркинг.ЗапуститьИзКаталога(Каталог);
...
&Тест
Процедура Тест_ПодключениеКлассаБенчмарков() Экспорт
	
	ПутьКФайлу = "./tests/fixtures/Классы/БенчмаркНесуществующий.os";
	Объект = Бенчмаркинг.ПодключитьКласс(ПутьКФайлу);

	Ожидаем.Что(Объект).ЭтоНеопределено();
