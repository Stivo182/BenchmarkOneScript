...
&Тест
Процедура ТестДолжен_УстановитьИсточникПараметровВКонфигурации() Экспорт
	...
	// Действие
	Конфигурация.ДобавитьИсточникПараметров("ОбщееЗначение", "Получить");
...
&Тест
Процедура ТестДолжен_ОчиститьИсточникиПараметровВКонфигурации() Экспорт
...
Процедура ТестДолжен_ЗапуститьСИсточникомПараметровДляОбщегоПоляИзСтруктуры() Экспорт
	...
	// Проверка
	Ожидаем.Что(Результат.Запуски).ИмеетДлину(1);

	Значение = Результат.Запуски[0].Кейс.Параметры()[0].Значение();
	Ожидаем.Что(Значение).ИмеетТип("Структура");
	Ожидаем.Что(Значение.Парам1).Равно(13);
	Ожидаем.Что(Значение.Парам2).Равно(14);
...
Процедура ТестДолжен_ВыброситьИсключениеСНевернымТипомЭлементаКоллекцииИсточникаПараметров() Экспорт
	...
	Ожидаем
		...
		.ВыбрасываетИсключение("для передачи параметров в метод бенчмарка <БенчмаркДваПараметра>");
...
Процедура ТестДолжен_ВыброситьИсключениеСОтсутствующимПараметромВСтруктуреИсточникаПараметров() Экспорт
	...
	Ожидаем
		.Что(Бенчмаркинг)
		.Метод("Запустить", ПараметрыМетода)
		.ВыбрасываетИсключение("Отсутствует параметр <Парам2> метода бенчмарка <БенчмаркДваПараметра> в переданной коллекции");
