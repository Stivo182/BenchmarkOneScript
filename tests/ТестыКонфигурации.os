...
Процедура ТестДолжен_ДобавитьОбработчикСобытия() Экспорт
	...
	ОбработчикСобытия = Новый ОбработчикСобытияБенчмарка(
		"БезАннотации",
		СобытияБенчмарков.ПередКаждым);

	Конфигурация = Новый КонфигурацияБенчмарков(Тип);
	ТестированиеБенчмарков.НастроитьКонфигурациюПодТесты(Конфигурация);

	Конфигурация.ДобавитьОбработчикСобытия(ОбработчикСобытия);
	Конфигурация.ДобавитьПараметр("ПутьКЛогу", ПутьКЛогу);
...
Процедура ТестДолжен_ОбъединитьПустуюКонфигурациюСПолностьюЗаполненной() Экспорт
	...
	ПередаваемаяКонфигурация = Новый КонфигурацияБенчмарков()
		...
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик1", СобытияБенчмарков.ПередВсеми))
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик2", СобытияБенчмарков.ПослеКаждого))
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик3", СобытияБенчмарков.ПослеКаждого))
...
Процедура ТестДолжен_ОбъединитьЗаполненныеКонфигурации() Экспорт
	...
	КонфигурацияПриемник = Новый КонфигурацияБенчмарков()
		...
		.ДобавитьКолонку(КолонкиОтчетаБенчмарков.НижнийКвартиль)
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик1", СобытияБенчмарков.ПередВсеми))
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик2", СобытияБенчмарков.ПослеВсех))
	...
	КонфигурацияИсточник = Новый КонфигурацияБенчмарков()
		...
		.ДобавитьКолонку(КолонкиОтчетаБенчмарков.ВерхнийКвартиль)
		.ДобавитьЭкспортер(ЭкспортерыРезультатовБенчмарков.Html)
		.ДобавитьОбработчикСобытия(Новый ОбработчикСобытияБенчмарка("Обработчик3", СобытияБенчмарков.ПослеВсех))
