...
Процедура ТестДолжен_ДобавитьОбработчикСобытия() Экспорт
	...
	Конфигурация.ДобавитьОбработчикСобытия("БезАннотации", СобытияБенчмарков.ПередКаждым);
	Конфигурация.ДобавитьПараметр("ПутьКЛогу", ПутьКЛогу);
...
Процедура ТестДолжен_ВыброситьИсключениеНаНесуществующемОбработчикеСобытия() Экспорт
	...
	Конфигурация.ДобавитьОбработчикСобытия("НесуществующийОбработчик", СобытияБенчмарков.ПередКаждым);
...
Процедура ТестДолжен_ОбъединитьПустуюКонфигурациюСПолностьюЗаполненной() Экспорт
	...
	ПередаваемаяКонфигурация = Новый КонфигурацияБенчмарков()
		...
		.ДобавитьОбработчикСобытия("Обработчик2", СобытияБенчмарков.ПослеКаждого)
		.ДобавитьОбработчикСобытия("Обработчик3", СобытияБенчмарков.ПослеКаждого)
	...
	Ожидаем.Что(НоваяКонфигурация.ОбработчикиСобытия(СобытияБенчмарков.ПослеКаждого)[0].ИмяМетода(), "Обработчики событий").Равно("Обработчик2");
	Ожидаем.Что(НоваяКонфигурация.ОбработчикиСобытия(СобытияБенчмарков.ПослеКаждого)[1].ИмяМетода(), "Обработчики событий").Равно("Обработчик3");
...
Процедура ТестДолжен_ОбъединитьЗаполненнуюКонфигурациюСПустой() Экспорт
	...
	Ожидаем.Что(ЗаполненнаяКонфигурация.ОбработчикиСобытия(СобытияБенчмарков.ПослеКаждого)[0].ИмяМетода(), "Обработчики событий").Равно("Обработчик2");
	Ожидаем.Что(ЗаполненнаяКонфигурация.ОбработчикиСобытия(СобытияБенчмарков.ПослеКаждого)[1].ИмяМетода(), "Обработчики событий").Равно("Обработчик3");
