...
Процедура Тест_ДолженПроверитьСериализациюКонфигурацииJson() Экспорт
	...
	Сериализатор = Новый СериализаторБенчмарков();
	Сериализатор.ЗаписатьВJson(Конфигурация, ИмяФайла);
	КонфигурацияПрочитанная = Сериализатор.ПрочитатьИзJson(ИмяФайла);
