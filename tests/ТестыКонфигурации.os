...
Процедура Тест_ОбъединениеЗаполненныхКонфигураций() Экспорт
	...
	Ожидаем.Что(ОбработчикиСобытий.Найти(СобытияБенчмарков.ПередВсеми, "Событие"), "Обработчики событий").ЭтоНеопределено();
	Ожидаем.Что(ОбработчикиСобытий.Найти(СобытияБенчмарков.ПослеВсех, "Событие").Обработчик, "Обработчики событий").Равно("Обработчик2");
