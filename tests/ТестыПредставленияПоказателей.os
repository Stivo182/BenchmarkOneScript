...
Процедура Тест_ПредставлениеБулево() Экспорт
	...
	ТестовыйНабор = Новый Соответствие();
	ТестовыйНабор[Истина] = "True";
	ТестовыйНабор[Ложь] = "False";

	Для Каждого Строка Из ТестовыйНабор Цикл

		Значение = Строка.Ключ;
		Представление = Строка.Значение;
...
&Тест
Процедура Тест_ПредставлениеБулевоФорматнойСтрокой() Экспорт

	ФорматнаяСтрока = "БЛ=No; БИ=Yes";

	ТестовыйНабор = Новый Соответствие();
	ТестовыйНабор[Истина] = "Yes";
	ТестовыйНабор[Ложь] = "No";
	...
	Для Каждого Строка Из ТестовыйНабор Цикл
		...
		Результат = ПредставленияПоказателейБенчмарков.Представление(Значение, , ФорматнаяСтрока);
		Ожидаем.Что(Результат).Равно(Представление);
